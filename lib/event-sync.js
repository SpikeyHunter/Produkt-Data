...
async function venueIndex(supabase) {
  const { data, error } = await supabase
    ...
    .select('id, name, display_name, enabled, ra_venue_id, evenko_slug, tixr_group_slug, '
          + 'ra_area_id, ra_promoter, internal_venues, platform, color, group_id');
...
async function syncInternal(supabase, { from, to, dryRun = false, log = console.log } = {}) {
  ...
  log(`  [sync:internal] ${from}..${to}: ${candidates.length} of our own events `
...
module.exports = { syncRA, syncEvenko, syncInternal, priceRA, bindAndWrite, fingerprint, toRow, venueIndex, resolveVenue,
