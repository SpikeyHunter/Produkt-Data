...
async function extractEvents({ content, source, recipe = {}, hint = null }) {
  ...
  const reply = await llmComplete(EXTRACT_SYSTEM, [
 ...
 */
async function authorRecipe({ url, pageSample, networkSamples = [], selectedElements = [], messages = [], currentRecipe = null }) {
  const evidence = {
    url,
  ...
  const reply = await llmComplete(AUTHOR_SYSTEM, turns, { maxTokens: 3000 });
