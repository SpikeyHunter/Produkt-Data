 ...
 */
async function refineDetections({ url, routes = [], events = [], corrections = [], rejected = [], feedback = '' }) {
  const evidence = {
    url,
  ...
  parsed.routes = parsed.routes.map(r => ({
    ...
    field_hints: Object.fromEntries(
      Object.entries(r.field_hints && typeof r.field_hints === 'object' ? r.field_hints : {})
        .filter(([, v]) => typeof v === 'string' && v.trim() !== '')),
    // Asked for a line of text, the model often answers with an object.
    // Both are fine as a worked example; the client stores a string.
    example_event: r.example_event == null ? null
      : (typeof r.example_event === 'string' ? r.example_event : JSON.stringify(r.example_event)),
  }));
  return parsed;
}
...
module.exports = { extractEvents, extractFromFlyers, authorRecipe, proposeDetections, refineDetections, parseJSON };
