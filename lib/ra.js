...
// The argument that makes this work. Selecting `tickets` bare returns an
// empty array on every public event, which reads exactly like permission
// gating — it is not. With queryType AVAILABLE, RA hands over the full
// retail ladder unauthenticated: "Early bird 12 CAD", "General admission 15".
// That single argument is the difference between third-party pricing being
// free and it needing a per-vendor scraper.
const TICKETS_QUERY = `query EVENT_TICKETS($id: ID) {
  event(id: $id) {
    id
    tickets(queryType: AVAILABLE) {
      id title priceRetail validType isAddOn onSaleFrom onSaleUntil
      currency { code }
    }
  }
}`;

// Not real admission: comps, guest list, and the add-ons that would otherwise
// set a nonsense floor on an event's price range.
const NON_PUBLIC = /comp|guest ?list|artist|staff|invite|donation|rsvp|add[- ]?on|merch|locker|coat/i;

/**
 * The price ladder for one event. Returns null rather than an empty ladder
 * when nothing is on sale, so callers can tell "no tickets" from "free".
 */
async function fetchTickets(eventId) {
  const data = await raQuery(TICKETS_QUERY, { id: String(eventId) },
    { operationName: 'EVENT_TICKETS' });
  const raw = data?.event?.tickets || [];

  const tiers = raw
    .filter(t => t && !t.isAddOn && !NON_PUBLIC.test(t.title || ''))
    .map(t => ({
      name: t.title || null,
      price: Number(t.priceRetail),
      currency: t.currency?.code || 'CAD',
      category: /vip|table/i.test(t.title || '') ? 'VIP' : 'GA',
      sold_out: false,
    }))
...
module.exports = {
  raQuery, resolveArea, listEvents, listEventsChunked, listVenues, mapEvent, fetchTickets,
