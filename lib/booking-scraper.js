 ...
 */
async function collectEvents(source, { renderedHTML = null, force = false,
                                       routes: only = null, log = console.log } = {}) {
  const recipe = source.recipe || {};
  const routes = only || (Array.isArray(recipe.routes) && recipe.routes.length
    ? recipe.routes
...
/**
 * A detection is either UPCOMING (the next few shows — cheap, changes daily)
 * or BACKFILL (the archive — expensive, barely changes). Reading 74 flyers
 * with vision every night to re-learn events from 2024 is money for nothing,
 * so the archive runs only when it is due or explicitly asked for.
 */
const BACKFILL_EVERY_DAYS = 30;

function selectRoutes(source, backfill = 'auto') {
  const routes = source.recipe?.routes;
  if (!Array.isArray(routes) || !routes.length) return { routes: null, ranBackfill: false };

  const isBackfill = r => (r.role || 'upcoming') === 'backfill';
  if (backfill === 'force') {
    return { routes, ranBackfill: routes.some(isBackfill) };
  }
  if (backfill === 'skip') {
    return { routes: routes.filter(r => !isBackfill(r)), ranBackfill: false };
  }

  const last = source.backfill_at ? new Date(source.backfill_at).getTime() : 0;
  const due = !last || (Date.now() - last) > BACKFILL_EVERY_DAYS * 86_400_000;
  return {
    routes: due ? routes : routes.filter(r => !isBackfill(r)),
    ranBackfill: due && routes.some(isBackfill),
  };
}

/** Scrapes one source and upserts what it finds. */
async function scrapeSource(supabase, source, { renderedHTML = null, dryRun = false,
                                               backfill = 'auto', log = console.log } = {}) {
  const started = Date.now();
  // A test run means "show me what these detections do", so run all of them.
  const picked = selectRoutes(source, dryRun ? 'force' : backfill);
  try {
    if (!dryRun) {
    ...
    if (picked.routes && !picked.routes.length) {
      log(`  [booking] ${source.name}: nothing due (archive already backfilled)`);
      if (!dryRun) {
        await supabase.from('mktg_booking_source').update({
          last_sync_at: new Date().toISOString(), last_status: 'ok', last_error: null,
          updated_at: new Date().toISOString(),
        }).eq('id', source.id);
      }
      return { ok: true, count: 0, events: [], nothingDue: true };
    }

    const collected = (source.kind === 'internal')
      ? { events: await collectInternalEvents(supabase, source, { log }), hash: null }
      : await collectEvents(source, { renderedHTML, force: dryRun, routes: picked.routes, log });
    ...
    await supabase.from('mktg_booking_source').update({
      ...
      // Only stamp it when the archive genuinely ran, or it would go quiet
      // for a month after an upcoming-only sync.
      ...(picked.ranBackfill ? { backfill_at: new Date().toISOString() } : {}),
      updated_at: new Date().toISOString(),
    }).eq('id', source.id);
...
module.exports = { scrapeSource, scrapeAll, collectEvents, collectInternalEvents,
                   buildContent, fetchText, projectRow, absolute, selectRoutes };
