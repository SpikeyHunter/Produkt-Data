...
const MAX_CONTENT = 90_000;   // chars handed to the model per request
...
function buildContent(html, { url, listSelector, log = null }) {
  ...
  if (json.length) {
    const dump = JSON.stringify(json).slice(0, MAX_CONTENT * 0.6);
 ...
 */
async function collectEvents(source, { renderedHTML = null, log = console.log } = {}) {
  ...
  if (mode === 'api' && recipe.api?.url) {
    ...
    if (payload === null) {
      ...
      content = buildContent(raw, { url: recipe.api.url, listSelector: recipe.list_selector, log });
    } else {
    ...
    content = buildContent(html, { url: source.url, listSelector: recipe.list_selector, log });
  }
  ...
  log(`  [booking] ${source.name}: ${content.length} chars via ${mode}`);
  const events = await extractEvents({ content, source, recipe });
  ...
  if (recipe.detail?.follow_links && events.length) {
    ...
    for (let i = 0; i < limit; i++) {
      ...
      try {
        ...
        const detailContent = buildContent(detailHtml, { url: target, listSelector: recipe.detail.list_selector });
...
async function scrapeSource(supabase, source, { renderedHTML = null, dryRun = false, log = console.log } = {}) {
  ...
  try {
    ...
    const events = await collectEvents(source, { renderedHTML, log });
    const rows = events
