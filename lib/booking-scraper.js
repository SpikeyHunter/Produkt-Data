...
async function runRoute(source, route, { renderedHTML = null, force = false, log = console.log } = {}) {
  ...
  let content, structured = [], hash = null, rawHTML = null;
  ...
  if ((recipe.detail?.follow_links || recipe.follow_links) && events.length) {
    const limit = Math.min(recipe.detail.limit || 12, events.length);
    for (let i = 0; i < limit; i++) {
      const target = events[i]?.url;
      if (!target) continue;
      try {
        const detailHtml = await fetchText(absolute(target, routeURL));
