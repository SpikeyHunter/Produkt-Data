...
/**
 * Builds a user message whose content mixes text and images. Anthropic takes
 * image blocks with a URL source, so flyers can be read straight from the
 * CDN without downloading them first.
 */
function visionMessage(text, imageUrls = []) {
  const content = imageUrls.slice(0, 20).map(url => ({
    type: 'image',
    source: { type: 'url', url },
  }));
...
/** Enough room for a thinking pass plus a short answer. */
const MIN_ANSWER_BUDGET = 2500;

async function llmComplete(system, messages, { maxTokens = 2000, timeout = 60000, model = null } = {}) {
  if (ANTHROPIC_API_KEY) {
    const { data } = await axios.post(
      'https://api.anthropic.com/v1/messages',
      {
        model: model || ANTHROPIC_MODEL || 'claude-sonnet-5',
        // Thinking tokens are spent out of THIS budget before a word of the
        // answer is written, so a budget sized for the prose alone (400 for a
        // 150-word insight) gets consumed entirely by thinking and returns
        // nothing. Floor it. Output is billed per token actually generated,
        // and the prompts cap the prose, so headroom is free.
        max_tokens: Math.max(maxTokens, MIN_ANSWER_BUDGET),
    ...
    if (!text && data.stop_reason === 'max_tokens') {
      throw new Error(`Claude spent all ${Math.max(maxTokens, MIN_ANSWER_BUDGET)} tokens thinking `
        + 'and never wrote an answer — raise maxTokens for this call.');
    }
    return text;
  }
...
async function chatWithAnalyst(context, messages) {
  ...
  return llmComplete(CHAT_SYSTEM_PROMPT, turns, { maxTokens: 3000 });
}

module.exports = { classifyContent, classifyAd, generateProjectInsight, chatWithAnalyst, llmComplete, visionMessage };
