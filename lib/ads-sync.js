...
async function fetchAdsCatalog(supabase, { after = null, q = null } = {}) {
  const account = await resolveAccountId();
  // NO nested insights here — they made catalog pages take seconds each.
  const params = {
  ...
  if (q) {
    params.filtering = JSON.stringify([
      { field: 'ad.name', operator: 'CONTAIN', value: q },
    ]);
  }
  ...
  try {
    ...
    data = await graphGet(`/${account}/ads`, { ...params, limit: 12 });
  }

  const raw = data.data || [];
  const ids = raw.map(a => a.id);
  const { data: existing } = ids.length
    ? await supabase.from('mktg_ads').select('ad_id').in('ad_id', ids)
    : { data: [] };
...
/** Fast path: upsert the rows and RETURN — previews + AI suggestions finish
 *  in the background so the app shows the ads immediately. */
  ...
  setImmediate(async () => {
    try {
      ...
      await mapLimit(rows, 3, async (row) => {
        ...
        if (!inherited) await classifyAd(supabase, row, { log });
      });
      log(`[ads] import post-processing done (${rows.length} ads)`);
    } catch (err) {
      log(`[ads] import post-processing error: ${err.message}`);
    }
  });

  log(`[ads] imported ${rows.length} ad(s) — previews/AI continuing in background`);
