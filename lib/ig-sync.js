...
async function refreshDue(supabase, log) {
  const { data: due, error } = await supabase
    .from('mktg_ig_stats')
    .select('ig_media_id, media_product, media_type, posted_at, stop_at, stats_timeline, reach, views, likes, comments, shares, saves, error_count, meta')
    .eq('is_final', false)
  ...
  await mapLimit(due, 6, async (row) => {
    ...
    try {
      ...
      // Children only matter for carousels — asking for them on every story
      // meant re-fetching media each cycle AND clobbering the story extras
      // written just above with a stale meta.
      const needsPreview = !row.meta?.preview_path;
      const needsChildren = row.media_type === 'CAROUSEL_ALBUM' && !row.meta?.children;
      if (needsPreview || needsChildren) {
        try {
          const m = await graphGet(`/${row.ig_media_id}`, { fields: MEDIA_FIELDS });
          const meta = { ...(row.meta || {}), ...(update.meta || {}) };
          if (needsPreview) {
            const url = previewUrl(m);
