...
async function refreshDue(supabase, log) {
  ...
  await mapLimit(due, 6, async (row) => {
    ...
    try {
      ...
      // Self-heal: capture a preview if the first attempt failed/was skipped.
      if (!row.meta?.preview_path) {
        try {
          const m = await graphGet(`/${row.ig_media_id}`, { fields: MEDIA_FIELDS });
          const url = previewUrl(m);
          if (url) {
            const path = await captureMedia(supabase, row.ig_media_id, url);
            await supabase.from('mktg_ig_stats')
              .update({ meta: { ...(row.meta || {}), preview_path: path } })
              .eq('ig_media_id', row.ig_media_id);
            log(`  🖼️ preview healed for ${row.ig_media_id}`);
          }
        } catch { /* media gone or still no preview — try again next cycle */ }
      }
    } catch (e) {
...
module.exports = { runIgCycle, fetchCatalog, importMediaIds };
